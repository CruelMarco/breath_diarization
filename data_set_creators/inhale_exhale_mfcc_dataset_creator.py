...
for j in tqdm(wav_files) :
    ...
    xx_end_idx = end_col[xx_idx]
    
    xx_st_sam = list(np.ceil(ii_st_idx * fs))
    ...
    for q in range(len(xx_st_sam)) :
        ...
        delta_xx_chunk_mfcc_df = pd.DataFrame(delta , columns = ['F0_d' , 'F1_d' , 'F2_d' , 'F3_d' , 'F4_d' , 'F5_d' , 'F6_d' , 'F7_d' , 'F8_d' , 'F9_d' , 'F10_d' , 'F11_d' , 'F12_d'])

        delta2 = np.array(np.transpose(librosa.feature.delta(mfcc_xx, order=2)))
        
        delta2_xx_chunk_mfcc_df = pd.DataFrame(delta2 , columns = ['F0_dd' , 'F1_dd' , 'F2_dd' , 'F3_dd' , 'F4_dd' , 'F5_dd' , 'F6_dd' , 'F7_dd' , 'F8_dd' , 'F9_dd' , 'F10_dd' , 'F11_dd' , 'F12_dd'])
        
        xx_chunk_mfcc_df = pd.DataFrame(xx_chunk_mfcc_df , columns = ['F0' , 'F1' , 'F2' , 'F3' , 'F4' , 'F5' , 'F6' , 'F7' , 'F8' , 'F9' , 'F10' , 'F11' , 'F12'])
        
        index_xx_df = pd.DataFrame([xx_idx[q] ]*xx_chunk_mfcc_df.shape[0], columns = ['phon_idx'])
        
        mfcc_xx_delta_delta2 = pd.concat([index_xx_df , xx_chunk_mfcc_df, delta_xx_chunk_mfcc_df, delta2_xx_chunk_mfcc_df],axis=1, join='inner')
        
        #print(mfcc_ii_delta_delta2)
        
        sub_xx_mfcc_df.append(mfcc_xx_delta_delta2)
    
    sub_xx_mfcc_df = pd.concat(sub_xx_mfcc_df, ignore_index=True)
     
    sub_xx_mfcc_df.insert(0,'Name' , name , True)
    
    phon_xx = "xx"
    
    sub_xx_mfcc_df['phon'] = phon_xx
    ...
    mfcc_df_dir = os.path.join(mfcc_store_dir , mfcc_name)
    
    sub_ii_xx_mfcc_df.to_csv(mfcc_df_dir)
